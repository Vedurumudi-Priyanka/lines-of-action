...
class BoardWidget extends Pad {
    ...
    /**
     * sOrigin is keeping track of the square that had been pressed
     * in order to link mouse pressed and mouse released. Because
     * it is important to keep track of the square that had been
     * pressed and the square in which you released on.
     */
    private Square sOrigin = null;
    ...
    private void mouseReleased(Square s) {
        if (sOrigin != null && sOrigin.isValidMove(s)
                && sOrigin.index() != s.index()) {
            Move m = Move.mv(sOrigin, s);
            _board.makeMove(m);
        }
        repaint();
    }
    ...
    /**
     * Function call led from the menu button to result
     * in the retracting of the move, calls the
     * backend from Board.java to retract that move.
     */
    void retractMove() {
        _board.retract();
        repaint();
